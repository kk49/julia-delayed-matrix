...
function demat_test()
    ...
    t2 = @elapsed for i = 1:tN a = b+c.*d + 1.0f0 end
    #@time a = b + 1.0
...
#    println("  Time to only run one iteration (replace second operand of top operator): $et seconds")
    ...
    t3 = @elapsed for i = 1:tN ad[] = bd+cd.*dd + 1.0f0 end
    #@time ad[] = bd + 1.0 
    println("Elapsed time: $t3")

    println("#4 Delayed Expression (CUDA):")
    t4 = @elapsed for i = 1:tN ac[] = bc+cc.*dc + 1.0f0 end
    #@time ad[] = bd + 1.0f0
